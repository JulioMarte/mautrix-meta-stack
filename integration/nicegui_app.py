...
def _is_interactive_recaptcha_step(step: dict[str, Any]) -> bool:
    if str(step.get("type") or "") != "cookies":
        return False
    cookies = step.get("cookies") or {}
    for field in cookies.get("fields") or []:
        if not isinstance(field, dict) or field.get("id") != "recaptcha_token":
            continue
        for source in field.get("sources") or []:
            if (
                isinstance(source, dict)
                and source.get("type") == "special"
                and source.get("name") == "recaptcha_token"
            ):
                return bool(cookies.get("extract_js"))
    return False
        ...
        if saved_step:
            ...
            with ui.card().classes("w-full p-6 border border-amber-100"):
                ...
                if step_type == "cookies":
                    ...
                    interactive_recaptcha = _is_interactive_recaptcha_step(saved_step)
                    if interactive_recaptcha:
                        ui.label(
                            "Facebook requiere un reCAPTCHA interactivo. Este desafío no puede retransmitirse como una imagen: "
                            "debe ejecutarse en un webview con el origen que espera Facebook."
                        ).classes("text-slate-700 mt-3")
                        ui.label(f"Sitio del desafío: {url}").classes("text-sm text-slate-500")
                        ui.label(
                            "Abre el helper seguro. Resolverás el desafío allí y el helper entregará únicamente el token resultante al bridge."
                        ).classes("text-blue-700 font-medium mt-2")
                    else:
                        ui.label(
                            "Elegiste un método web. La versión actual de mautrix necesita datos de una sesión de Facebook y un "
                            "navegador normal no puede entregarlos de forma segura al panel por las restricciones de origen y cookies HttpOnly."
                        ).classes("text-slate-700 mt-3")
                    ...
                    if attachments:
                        with ui.column().classes("w-full gap-2 mt-3"):
                            ...
                            for attachment in attachments:
                                ...
                                elif content and mimetype.startswith("audio/"):
                                    has_audio = True
                                    ui.audio(f"data:{mimetype};base64,{content}").classes("w-full max-w-md")
                            if has_image and has_audio:
                                ui.label(
                                    "Escribe los caracteres de la imagen. Si no puedes leerla, usa la alternativa de audio."
                                ...
                                ui.label(
                                    "Escucha el audio y escribe los caracteres indicados."
                                ).classes("text-sm text-slate-500")
