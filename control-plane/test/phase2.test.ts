...
function fixture() {
  ...
  const profileA = egress.create({ provider: "fixture", scheme: "socks5", host: "proxy-a.test", port: 1080, username: "alice", secretRef: "env:EGRESS_PROXY_A", country: "US", region: null, stickySessionId: "sticky-a", expectedExitIp: null, status: "healthy" });
  const profileB = egress.create({ provider: "fixture", scheme: "socks5", host: "proxy-b.test", port: 1080, username: "bob", secretRef: "env:EGRESS_PROXY_B", country: "US", region: null, stickySessionId: "sticky-b", expectedExitIp: null, status: "healthy" });
...
describe("Phase 2 resolver domain", () => {
  test("sticky assignments remain stable and two accounts resolve differently", () => {
    const f = fixture(); const resolver = new EgressResolver(f.connections, f.egress, new EnvironmentSecretProvider({ EGRESS_PROXY_A: "secret-a", EGRESS_PROXY_B: "secret-b" }));
  ...
  test("conflicting identity fails closed", () => {
    ...
    const resolver = new EgressResolver(f.connections, f.egress, new EnvironmentSecretProvider({ EGRESS_PROXY_A: "a", EGRESS_PROXY_B: "b" }));
...
describe("Phase 2 internal API", () => {
  test("resolver requires internal authentication and returns confidential proxy only when authorized", async () => {
    const f = fixture(); const app = createApp(db!, "admin-token-123456789", "internal-token-123456789012345", { EGRESS_PROXY_A: "canary-secret", EGRESS_PROXY_B: "secret-b" });
  ...
  test("assignment management is audited without secret material", async () => {
    ...
    const profile = egress.create({ provider: "fixture", scheme: "socks5", host: "proxy.test", port: 1080, username: "u", secretRef: "env:EGRESS_PROXY_CANARY", country: null, region: null, stickySessionId: null, expectedExitIp: null, status: "healthy" });
    const app = createApp(database, "admin-token-123456789", "internal-token-123456789012345", { EGRESS_PROXY_CANARY: "raw-secret-canary" });
