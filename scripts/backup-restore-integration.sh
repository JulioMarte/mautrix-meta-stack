...
DC=(docker compose -f compose.yaml -f compose.control-plane.yaml -f compose.phase6.yaml -f compose.backup-restore.yaml)
...
# Seed every state class that the deployment contract requires a backup to retain.
run_bun <<'BUN'
...
const connection = (await post("/api/v1/meta-connections", {
  tenantId: tenant.id,
  matrixOwnerMxid: "@backup:matrix.example.com",
  metaAccountId: "meta-backup-1",
  mautrixLoginId: "login-backup-1",
})).data;
const binding = (await post("/api/v1/chatwoot-bindings", {
  tenantId: tenant.id,
  chatwootAccountId: "91",
  chatwootInboxId: "92",
  apiBaseUrl: "http://chatwoot-double:8080",
  credentialRef: "env:CHATWOOT_CI_TOKEN",
  status: "active",
})).data;
...
const inbound = {
  ...
  roomId: "!backup:matrix.example.com",
  remoteThreadId: "backup-thread",
  remoteContactId: "backup-contact",
  eventId: "$backup-event-1",
  senderId: "backup-contact",
  text: "backup state seed",
...
await Bun.write("/data/backup-acceptance.json", JSON.stringify({ tenantId: tenant.id, profileId: profile.id, connectionId: connection.id, bindingId: binding.id, conversationId: conv.id, chatwootConversationId: conv.chatwoot_conversation_id }));
db.close();
BUN

# A cold backup archives the entire SQLite state directory so WAL/journal files are
# retained if present. The application must be stopped before the archive is made.
...
# Destroy the original volume, create a fresh one, restore the archive, then let
# the normal data-init service repair ownership before starting the application.
"${DC[@]}" down -v --remove-orphans >/dev/null
"${DC[@]}" run --rm control-plane-backup-helper 'set -eu; rm -rf /data/* /data/.[!.]* /data/..?* 2>/dev/null || true; tar -C /data -xzf /backup/control-plane-data.tgz; test -s /data/control-plane.db'
"${DC[@]}" up -d chatwoot-double matrix-client-double control-plane
wait_healthy chatwoot-double 60
wait_healthy matrix-client-double 60
wait_healthy control-plane 90

# Verify restored identity/routing/idempotency/audit state and prove a duplicate
# does not create another downstream Chatwoot side effect after restore.
run_bun <<'BUN'
const { Database } = await import("bun:sqlite");
...
# The archive itself must not contain environment-only proxy/API credentials.
archive_strings="$(tar -xOzf "$CONTROL_PLANE_BACKUP_DIR/control-plane-data.tgz" ./backup-acceptance.json 2>/dev/null || true)"
for secret in "$CONTROL_PLANE_ADMIN_TOKEN" "$CONTROL_PLANE_INTERNAL_TOKEN" "$CHATWOOT_CI_TOKEN" "$EGRESS_PROXY_BACKUP_PASSWORD"; do
  printf '%s' "$archive_strings" | grep -Fq "$secret" && { echo "runtime secret leaked into backup acceptance metadata" >&2; exit 1; }
done
